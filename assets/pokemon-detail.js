...
async function loadPokemon(id) {
    try {
        ...
        console.error("Erreur détécté lors du chargement du Pokémon", error);
        return false;
    }
}
...
function setTypeBackgroundColor(pokemon) {
    ...
    if (!color) {
        console.warn(`Couleur non définie pour le type: ${mainType}`);
        return;
    }
...
function displayPokemonDetails(pokemon) {
    ...
    types.forEach(({ type}) => {
        createAndAppendElement(typeWrapper, "p", {className: `body3-fonts type ${name}`, textContent: type.name,});
    });
    ...
    stats.forEach(({stat, base_stat}) => {
        ...
        createAndAppendElement(statDiv, "progress", {className: "progress-bar",value:base_stat, max: 251,});
    });
